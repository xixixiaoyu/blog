...
2.  **数据转换的“负担”**
    后端服务通常是为多个不同的客户端设计的，因此它返回的数据格式是“通用”的，甚至是“原始”的。前端拿到数据后，还需要自己做大量的转换、裁剪和格式化工作，才能适配 UI 组件的需求。
    比如，后端返回的时间戳，前端需要转换成 `YYYY-MM-DD` 格式；后端返回的用户 ID 列表，前端需要再请求一次才能拿到用户名。这让前端承担了本不该它承担的业务逻辑，变得臃肿不堪。

...
如果修改同样的配置，需要去多个微服务下手动去改，然后重启服务，很麻烦。

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1694007710205-133cf88c-f5ef-42f2-8af7-31b1d7fccbca.png)
...
![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1694007758983-e173e898-6a17-463a-a828-3a24dd89c5b2.png)
...
微服务 A 依赖了微服务 B， B 有 3 个节点，但跑起来以后，某个节点挂掉了，并且还新增了几个微服务 B 的节点。

这时候微服务 A 怎么知道微服务 B 有哪些节点可用呢？

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1694008412638-e74a17ad-266a-4337-bc9d-2e28b0d624f1.png)

也是需要一个单独的服务来管理，这个服务就是注册中心：

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1694008479689-2d057c3d-8a9f-480a-a2bd-b29bf346987c.png)

微服务在启动的时候，向注册中心注册，销毁的时候向注册中心注销，并且定时发心跳包来汇报自己的状态。

查找其他微服务的时候，去注册中心查一下这个服务的所有节点信息，然后再选一个来用，这个叫做服务发现。

这样微服务就可以动态的增删节点而不影响其他微服务了。

微服务架构的后端系统中，配置中心和注册中心是必备组件。
...
![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1694008714346-4198cc95-7be1-4a72-99aa-e1e2693a12c8.png)
...
![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1694008765054-b9756897-0840-4ee3-aadf-1696ceec6dd5.png)

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1694009091725-aa5ebce4-6e37-41d4-b89c-76ff76ba6b39.png)

输入容器名，映射 2379 端口到容器内的 2379 端口，设置 ETCD_ROOT_PASSWORD 环境变量，也就是指定 root 的密码。

然后 etcd server 的 docker 镜像成功跑起来了。

它带了一个 etcdctl 的命令行工具，可以作为客户端和 etcd server 交互。

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1694009167730-b058618b-7403-44f9-a046-3e3e38c38061.png)

常用的命令有这么几个，对 key value 的增删改查和 watch 变动：
...
```javascript
etcdctl get --user=root --password=yun key
```

如果不想每次都指定用户名密码，在 terminal 可以设置环境变量：

```javascript
export ETCDCTL_USER=root
export ETCDCTL_PASSWORD=yun
```
...
![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1694009381506-033ec793-fd70-47ce-8e8b-668d90161b3e.png)

然后 get 查询值：

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1694009426123-e90de70a-e8e2-4513-818e-c5b6047a235a.png)

也可以通过 --prefix 查询指定前缀的 key 的值：

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1694009467094-da8ccd61-9e7a-4267-9221-461940b2868d.png)
...
![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1694009545550-0dda7e04-4009-4066-8a72-6eef4d0ca121.png)

这样的 key-value 用来存储「服务名-链接信息」，那就是注册中心，
...
![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1694009814286-2da15042-862a-427f-a7ed-ddabf9012b29.png)

然后执行上面的 node 脚本：

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1694009837070-b0fed5cc-819c-43f9-a156-29cf24d5f004.png)

确实取到了 etcd server 中的值。

然后在 etcdctl 里 put 修改下 /services/a 的值：

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1694099885457-c98d19f2-c751-4ab7-8a39-cb3592a622e2.png)

在 node 脚本这里收到了通知：

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1694099864676-1dcf576b-bb9d-448b-8fd8-dc2cb5d9fd2b.png)

再 del 试下：

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1694099933899-9752c350-a074-459c-a5f8-fadcb5e60ba5.png)

也收到了通知：

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1694099970346-347913ca-4a0d-410e-8723-b515d3b4b3ba.png)
...
![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1694100181071-065c0e39-a23d-4f1c-9b8a-7a8e7ce70493.png)

然后就可以在这里存各种数据库连接信息、环境变量等各种配置。
...
注册时按照 /services/服务名/实例id 的格式来指定 key。
...
<font style="color:rgb(227, 214, 192);background-color:rgb(28, 28, 26);">跑起来确实能获得服务的所有节点信息：</font>

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1694100634918-3813997a-affe-4009-9ded-fe9103b19551.png)

当在 etcdctl 里 del 一个服务节点的时候，这里也能收到通知：

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1694100706199-cfb03c97-4e3b-42c2-a28a-c4c09a57cf68.png)

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1694100686478-e57f74bc-9cdf-4ae5-89ae-16d57533759c.png)

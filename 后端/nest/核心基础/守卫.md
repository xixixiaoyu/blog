...
提示：`Reflector.getAllAndOverride` 会同时读取方法和控制器层级的元数据（数组中的顺序决定覆盖优先级，通常方法级优先于控制器级）。如果你使用 `SetMetadata('roles', ...)` 而不是 `Reflector.createDecorator` 创建装饰器，同样可以把 `'roles'` 作为第一个参数传给 `getAllAndOverride` 来实现相同行为。

在开发后端服务的时候，我们经常会遇到一些重复性的需求：记录每个请求的日志、统计接口响应时间、统一处理返回数据格式，或者在某些情况下直接返回缓存数据而不执行复杂的业务逻辑。这些场景用 NestJS 的拦截器来处理很适合。

## 什么是拦截器？
拦截器本质上就是一个实现了 `NestInterceptor` 接口的类，你可以把它想象成应用中的"安检口"或者"加工站"。

它能够在请求到达你的核心业务逻辑之前或之后插入一些额外的操作。



## 拦截器能帮我们做什么？
+ **前后夹击**：在你的主要处理方法执行前后加上额外逻辑，比如打印日志、统计耗时等。
+ **数据变身**：改变方法返回的结果。比如把所有返回数据都包装在一个统一的 `data` 字段里。
+ **错误包装**：转换方法抛出的异常，把原始的数据库错误包装成用户友好的提示信息。
+ **功能增强**：给基础方法行为加点新花样，比如自动处理重复性任务。
+ **直接接管**：在特定条件下（比如缓存命中）完全不执行原方法，直接返回预设值。



## 拦截器的工作原理
每个拦截器都必须实现 `intercept` 方法，这个方法接收两个重要参数：

### context (ExecutionContext)
这个参数继承自 `ArgumentsHost`，能让你获取原始请求的各种信息。更重要的是，它能告诉你当前请求的"上下文环境" —— 是哪个控制器、哪个方法在处理请求，请求类型是 HTTP、WebSocket 还是 gRPC 等等。就像这次请求的"身份证"。

### next (CallHandler)
这个参数非常关键。它包含一个 `handle()` 方法。当你调用 `next.handle()` 时，就相当于对请求说"放行！"，让请求继续执行原本的路由处理函数。如果不调用它，请求就会被"卡住"，后续逻辑都不会执行。

`next.handle()` 返回的是一个 `Observable` 对象（来自 RxJS 库）。你可以把它想象成一个数据管道，利用 RxJS 提供的各种操作符（如 `pipe()`、`tap()`、`map()` 等）来对这个"流"进行加工处理。  


## 🌰 记录请求耗时
让我们看一个记录请求处理时间的拦截器：

```typescript
// logging.interceptor.ts
import { Injectable, NestInterceptor, ExecutionContext, CallHandler, Logger } from '@nestjs/common';
import { Observable } from 'rxjs';
import { tap } from 'rxjs/operators';

@Injectable()
export class LoggingInterceptor implements NestInterceptor {
  private readonly logger = new Logger(LoggingInterceptor.name);

  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    const request = context.switchToHttp().getRequest();
    const method = request.method;
    const url = request.url;
    this.logger.log(`[${method}] ${url} - 请求处理开始...`);

    const now = Date.now();
    return next
      .handle()
      .pipe(
        tap(() => 
          this.logger.log(`[${method}] ${url} - 请求处理完毕... 耗时 ${Date.now() - now} ms`)
        )
      );
  }
}
```

在这个例子中：

+ `next.handle()` 之前的代码会在目标方法执行前运行
+ `tap()` 操作符中的回调函数会在目标方法执行完毕后运行



## 如何使用拦截器？
使用 `@UseInterceptors()` 装饰器，你可以在不同范围内应用拦截器：

### 控制器范围
```typescript
// LoggingInterceptor 会应用到 CatsController 下的所有路由处理方法
@UseInterceptors(LoggingInterceptor)
@Controller('cats')
export class CatsController {
  @Get()
  findAll() {
    return '所有的小猫咪都在这里！';
  }

  @Post()
  create(@Body() catInfo: any) {
    return `创建了一只新猫：${catInfo.name}`;
  }
}
```

### 方法范围
```typescript
@Controller('cats')
export class CatsController {
  @Get()
  findAll() {
    return '所有的小猫咪都在这里！';
  }

  @UseInterceptors(LoggingInterceptor) // 只对 create 方法生效
  @Post()
  create(@Body() catInfo: any) {
    return `创建了一只新猫：${catInfo.name}`;
  }
}
```

### 全局范围
**方法一：**在 `main.ts` 文件里，直接使用 `app.useGlobalInterceptors()`。

```typescript
// main.ts
import { NestFactory } from '@nestjs/core';
import { AppModule } from './app.module';
import { LoggingInterceptor } from './logging.interceptor'; // 假设拦截器在这里

async function bootstrap() {
  const app = await NestFactory.create(AppModule);
  // 全局生效，但这样 new 出来的 LoggingInterceptor 不能注入其他依赖
  app.useGlobalInterceptors(new LoggingInterceptor()); 
  await app.listen(3000);
}
bootstrap();
```

这种方式简单直接，但有个缺点：如果你的 `LoggingInterceptor` 需要注入其他服务 (比如一个配置服务或数据库服务)，这种方式就搞不定了。

**方法二 (推荐)**：在模块中注册全局拦截器，这样支持依赖注入：

```typescript
// app.module.ts
import { Module } from '@nestjs/common';
import { APP_INTERCEPTOR } from '@nestjs/core';
import { LoggingInterceptor } from './logging.interceptor';

@Module({
  providers: [
    {
      provide: APP_INTERCEPTOR, // 这是一个固定的 Token，告诉 Nest 这是个全局拦截器
      useClass: LoggingInterceptor, // 指定用哪个拦截器类
    },
  ],
})
export class AppModule {}
```



## 数据转换：统一响应格式
很多时候我们希望 API 返回的数据有统一格式，比如所有成功响应都包装在 `data` 属性里：

```typescript
// transform.interceptor.ts
import { Injectable, NestInterceptor, ExecutionContext, CallHandler } from '@nestjs/common';
import { Observable } from 'rxjs';
import { map } from 'rxjs/operators';

// 定义一下我们期望的响应结构
export interface Response<T> {
  statusCode: number; // 通常 HTTP 状态码由框架处理，这里可以加业务状态码
  message?: string;
  data: T;
}

@Injectable()
export class TransformInterceptor<T> implements NestInterceptor<T, Response<T>> {
  intercept(context: ExecutionContext, next: CallHandler): Observable<Response<T>> {
    return next.handle().pipe(
      // map 操作符：拿到原始路由处理函数返回的数据 (data)，把它转换成新格式
      map(data => ({
        statusCode: context.switchToHttp().getResponse().statusCode, // 获取 HTTP 状态码
        message: '请求成功', // 或者根据实际情况设置
        data: data // 把原始数据放到 data 字段里
      }))
    );
  }
}
```

如果你的路由处理函数返回 `['橘猫', '狸花猫']`，应用这个拦截器后，客户端收到的响应会是：

```json
{
  "statusCode": 200,
  "message": "请求成功",
  "data": ["橘猫", "狸花猫"]
}
```

**请注意**：这种响应映射的拦截器，对于那些直接使用 `@Res()` 装饰器来操作底层响应对象 (比如 `res.send()` 或 `res.json()`) 的路由处理方法是**无效**的。因为一旦你用了 `@Res()`，你就全面接管了响应流程，Nest 的这种标准响应处理机制（包括拦截器对响应的修改）就会被绕过。



## 错误处理：优雅地捕获异常
当业务逻辑抛出错误时，拦截器可以优雅地接住并转换它们：

```typescript
// errors.interceptor.ts
import {
  Injectable,
  NestInterceptor,
  ExecutionContext,
  CallHandler,
  BadGatewayException,
  Logger,
} from '@nestjs/common';
import { Observable, throwError } from 'rxjs';
import { catchError } from 'rxjs/operators';

@Injectable()
export class ErrorsInterceptor implements NestInterceptor {
  private readonly logger = new Logger(ErrorsInterceptor.name);

  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    return next.handle().pipe(
      catchError(err => {
        this.logger.error(`糟糕，出错了！原始错误: ${err.message}`, err.stack);
        
        return throwError(() => new BadGatewayException('哦豁，服务器好像开小差了...请稍后再试！'));
      })
    );
  }
}
```

这样，即便是程序内部抛出了一个很原始的错误，经过这个拦截器的“包装”，客户端最终只会收到一个统一的、相对友好的 `BadGatewayException` 响应。



## 高级应用：缓存和超时处理
### 缓存实现
```typescript
// cache.interceptor.ts (这是一个非常简化的示例)
import { Injectable, NestInterceptor, ExecutionContext, CallHandler, Logger } from '@nestjs/common';
import { Observable, of } from 'rxjs'; // of 操作符可以创建一个立即发出指定值的 Observable
import { tap } from 'rxjs/operators';

// 假设我们有一个简单的内存缓存服务
const cache = new Map<string, any>();

@Injectable()
export class CacheInterceptor implements NestInterceptor {
  private readonly logger = new Logger(CacheInterceptor.name);

  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    const request = context.switchToHttp().getRequest();
    const cacheKey = `${request.method}_${request.url}`;
    
    const cachedResponse = cache.get(cacheKey);
    if (cachedResponse) {
      this.logger.log(`命中缓存 [${cacheKey}]，直接返回！`);
      // 如果缓存命中，就用 of() 直接创建一个包含缓存数据的 Observable 并返回
      // 这样就不会执行 next.handle()，也就是不会执行真正的路由处理函数了
      return of(cachedResponse); 
    }

    this.logger.log(`缓存未命中 [${cacheKey}]，继续执行...`);
    // 缓存未命中，正常执行路由处理函数
    return next.handle().pipe(
      tap(response => {
        // 将结果存入缓存，可以设置过期时间等复杂逻辑
        cache.set(cacheKey, response);
        this.logger.log(`已缓存响应数据 [${cacheKey}]`);
      })
    );
  }
}
```

在这个简化版 `CacheInterceptor` 中，如果缓存命中，我们就可以用 RxJS 的 `of()` 操作符直接创建一个包含缓存数据的 `Observable` 并返回，从而“截胡”掉原来的 `next.handle()` 调用，请求不会到达真正的业务逻辑处理函数。

### 超时处理
```typescript
// timeout.interceptor.ts
import { 
  Injectable, 
  NestInterceptor, 
  ExecutionContext, 
  CallHandler, 
  RequestTimeoutException, // Nest 内置的请求超时异常
  Logger,
} from '@nestjs/common';
import { Observable, throwError, TimeoutError } from 'rxjs'; // TimeoutError 是 RxJS 抛出的错误类型
import { catchError, timeout } from 'rxjs/operators';

@Injectable()
export class TimeoutInterceptor implements NestInterceptor {
  private readonly logger = new Logger(TimeoutInterceptor.name);

  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    return next.handle().pipe(
      timeout(5000), // 设置 5 秒超时，如果 5 秒内没响应，就会抛出 TimeoutError
      catchError(err => {
        if (err instanceof TimeoutError) {
          this.logger.warn('请求处理超时！');
          // 如果是超时错误，就抛出 Nest 内置的 RequestTimeoutException
          return throwError(() => new RequestTimeoutException('请求超时啦，请稍后再试！'));
        }
        // 如果是其他类型的错误，就继续向上抛，让其他错误处理机制接手
        return throwError(() => err);
      }),
    );
  }
}
```

当请求处理时间超过 5 秒，这个拦截器就会捕获到 `TimeoutError`，并将其转换为一个标准的 `RequestTimeoutException` 返回给客户端。





## RxJS 初识
RxJS 是一个组织异步逻辑的库，它有很多 operator，可以极大的简化异步逻辑的编写。

它是由数据源（observable）产生数据，经过一系列 operator 的处理，最后传给接收者。

比如这样：

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1687068181380-d1bee459-b838-4867-b2c3-d93ace9ab220.png)

调用 of 操作符创建一个 Observable，它发送了三个值 1、2、3。

使用 pipe 方法，将 map 操作符添加到 Observable 上，对每个输入值进行平方运算，得到一个新的 Observable，它发送了 1、4、9。

再次使用 pipe 方法，将 filter 操作符添加到 Observable 上，过滤掉所有偶数值，得到一个新的 Observable，它发送了 1、9。

最后，调用 subscribe 方法订阅这个 Observable，当它发送新值时，调用提供的回调函数输出结果。在这个例子中，回调函数输出了每个接收到的值。



继续看这段代码：

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1687068532575-0df9555c-0ead-4b1f-8dc2-fc7bea1ee8b6.png)

调用 of 操作符创建一个 Observable，它发送了三个值 1、2、3。

使用 pipe 方法，将 scan 操作符添加到 Observable 上，对每个输入值进行累加操作，得到一个新的 Observable，它在每次接收到值时都会发出累加结果。在这个例子中，第一个值是 1，第二个值是 1 + 2 = 3，第三个值是 1 + 2 + 3 = 6。

再次使用 pipe 方法，将 map 操作符添加到 Observable 上，对每个累加结果进行平均数计算，得到一个新的 Observable，它在每次接收到值时都会发出平均数结果。在这个例子中，第一个值是 1，第二个值是 (1 + 2) / 2 = 1.5，第三个值是 (1 + 2 + 3) / 3 = 2。

最后，调用 subscribe 方法订阅这个 Observable，当它发送新值时，调用提供的回调函数输出结果。在这个例子中，回调函数输出了每个接收到的平均数结果。



再来看节流、防抖：

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1687068935576-811a596a-c1c9-42b5-b435-637f7e36dad9.png)

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1687068960425-9f7ab213-f00c-4637-8e6c-cc327743c833.png)

可以在官网文档看到[所有的 operator](https://link.juejin.cn/?target=https%3A%2F%2Frxjs.dev%2Fguide%2Foperators%23creation-operators-1)。

如果异步逻辑复杂度高， RxJS 收益还是很高的。

Nest 的 interceptor 集成了 RxJS，可以用它来处理响应。



## Nest 中使用 RxJS
### map
map 操作符允许对从请求处理程序返回的数据进行转换。例如，可以使用它将数据包装在一个标准的响应对象中，该对象包含状态码、消息和数据。

生成一个 interceptor：

```bash
nest g interceptor map-test --flat --no-spec
```

使用 map operator 对 controller 返回的数据做一些修改：

```javascript
import {
  CallHandler,
  ExecutionContext,
  Injectable,
  NestInterceptor,
} from '@nestjs/common';
import { Observable } from 'rxjs';
import { map } from 'rxjs/operators';

@Injectable()
export class MapTestInterceptor implements NestInterceptor {
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    return next.handle().pipe(
      map((data) => {
        return {
          code: 200,
          message: 'success',
          data,
        };
      }),
    );
  }
}
```

controll 下启用：

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1687071404488-d423f7ec-e3b6-4a37-9bd7-d3d9578e4266.png)

请求接口：

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1687071422542-667a9a62-700f-42c8-bcc3-94790d2396f5.png)





### tap
tap 操作符允许在不修改数据流的情况下执行副作用操作，比如记录日志或更新缓存。

这对于在请求处理过程中添加额外的日志记录或调试信息非常有用。

再生成个 interceptor

```bash
nest g interceptor tap-test --flat --no-spec
```

使用 tap operator 来添加一些日志、缓存等逻辑：

```javascript
import { AppService } from './app.service';
import {
  CallHandler,
  ExecutionContext,
  Injectable,
  Logger,
  NestInterceptor,
} from '@nestjs/common';
import { Observable } from 'rxjs';
import { tap } from 'rxjs/operators';

@Injectable()
export class TapTestInterceptor implements NestInterceptor {
  constructor(private appService: AppService) {}

  private readonly logger = new Logger(TapTestInterceptor.name);

  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    return next.handle().pipe(
      tap((data) => {
        // 这里假装下更新缓存的操作
        this.appService.getHello();

        this.logger.log(`log something`, data);
      }),
    );
  }
}
```

在 controller 返回响应的时候记录一些东西。

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1687071750460-49205b36-9ded-452b-93bb-117a97be05a3.png)

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1687071724570-99459155-431b-400b-887f-3e063737aad6.png)



### catchError
catchError 操作符允许处理在请求处理程序中抛出的异常。

可以先在拦截器中捕获这些异常，并执行一些额外的逻辑，比如记录错误日志或返回自定义的错误响应。

生成 interceptor：

```bash
nest g interceptor catch-error-test --flat --no-spec
```

使用 catchError 处理抛出的异常：

```typescript
import {
  CallHandler,
  ExecutionContext,
  Injectable,
  Logger,
  NestInterceptor,
} from '@nestjs/common';
import { Observable, throwError } from 'rxjs';
import { catchError } from 'rxjs/operators';

@Injectable()
export class CatchErrorTestInterceptor implements NestInterceptor {
  private readonly logger = new Logger(CatchErrorTestInterceptor.name);

  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    return next.handle().pipe(
      // 使用 catchError 操作符来捕获异常
      catchError((err) => {
        this.logger.error(err.message, err.stack);

         // 重新抛出错误，让它可以被后续的错误处理器捕获
        return throwError(() => err);
      }),
    );
  }
}
```

在 controller 使用下：

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1687072039431-c5a1d7e6-b744-4ce7-ac02-dc5cffe62f3b.png)

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1687072018764-cf299ae1-ccbf-496d-9b6d-39d74c523f32.png)

这个 500 错误，是内置的 exception filter 处理的：

nest 控制台会打印两次错误，报错信息都是 xxx。

一次是我们在 interceptor 里打印的，一次是 exception filter 打印的。



### timeout
timeout 操作符允许为请求处理程序设置一个超时时间。

如果在指定的时间内没有收到响应，它将抛出一个 TimeoutError 异常。

可以结合 catchError 操作符来处理这个异常，并返回一个适当的超时响应。

创建 interceptor：

```typescript
nest g interceptor timeout --flat --no-spec
```

添加如下逻辑：

```typescript
import {
  Injectable,
  NestInterceptor,
  ExecutionContext,
  CallHandler,
  HttpException,
  HttpStatus,
} from '@nestjs/common';
import { Observable, TimeoutError, throwError } from 'rxjs';
import { catchError, timeout } from 'rxjs/operators';

@Injectable()
export class TimeoutInterceptor implements NestInterceptor {
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    // 设置超时时间为3秒
    const TIMEOUT = 3000;

    return next.handle().pipe(
      timeout(TIMEOUT), // 在3秒后如果还没有响应，则抛出TimeoutError
      catchError((error) => {
        if (error instanceof TimeoutError) {
          // 如果捕获到超时错误，则抛出HttpException
          return throwError(
            () => new HttpException('请求超时', HttpStatus.REQUEST_TIMEOUT),
          );
        }
        // 如果是其他类型的错误，则继续抛出
        return throwError(() => error);
      }),
    );
  }
}
```

这样 timeout 操作符会在 3s 没收到消息的时候抛一个错误。



在 controller 使用下：

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1687072435684-140990b4-f0e2-441b-ba06-28f69617a9ad.png)

浏览器访问，3s 后返回 408 响应：

![](https://cdn.nlark.com/yuque/0/2024/png/21596389/1708948340028-600f9581-150a-4ba0-a8dd-e8e02f8e5727.png)

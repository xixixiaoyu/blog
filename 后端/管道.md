## 管道是干什么的？
管道本质上就是一个带有 `@Injectable()` 装饰器的类，它实现了 `PipeTransform` 接口。管道主要负责两件事：

1. **数据转换 (Transformation)**：把输入数据变成我们需要的格式。比如将路由参数中的字符串 `"123"` 转换成数字 `123`。
2. **数据验证 (Validation)**：检查输入数据是否符合要求。如果数据有问题，直接抛出异常告诉客户端哪里出错了。

管道的执行时机很关键 —— 它们在控制器处理程序之前运行。

当请求进来时，Nest 会先把数据交给管道处理，处理完成后，控制器方法才会接收到（可能已经被处理过的）数据。这样设计让我们的控制器能更专注于核心业务逻辑。



## Nest 内置管道一览
`@nestjs/common` 包为我们提供了丰富的内置管道：

+ **ValidationPipe**：功能强大的验证管道，后面会详细介绍
+ **ParseIntPipe**：字符串转整数
+ **ParseFloatPipe**：字符串转浮点数
+ **ParseBoolPipe**：字符串转布尔值
+ **ParseArrayPipe**：字符串转数组
+ **ParseUUIDPipe**：验证 UUID 格式
+ **ParseEnumPipe**：验证枚举值
+ **DefaultValuePipe**：提供默认值
+ **ParseFilePipe**：文件上传验证

提示：如果需要将字符串解析为 `Date`，Nest 并未内置 `ParseDatePipe`，可以通过实现自定义管道来完成类似功能。

让我们看个 🌰，使用 `ParseIntPipe` 处理路由参数：

```typescript
import { Controller, Get, Param, ParseIntPipe, HttpStatus } from '@nestjs/common';
import { CatsService } from './cats.service';

@Controller('cats')
export class CatsController {
  constructor(private catsService: CatsService) {}

  @Get(':id')
  async findOne(@Param('id', ParseIntPipe) id: number) {
    // 这里 id 肯定是 number 类型
    console.log(typeof id); // 输出 'number'
    return this.catsService.findOne(id);
  }
}
```

如果有人请求 `GET /cats/abc`，因为 `"abc"` 无法转换成数字，`ParseIntPipe` 会自动抛出 400 错误：

```json
{
  "statusCode": 400,
  "message": "Validation failed (numeric string is expected)",
  "error": "Bad Request"
}
```

我们还可以自定义错误状态码：

```typescript
@Get(':id')
async findOne(
  // 传递的是类实例
  @Param('id', new ParseIntPipe({ errorHttpStatusCode: HttpStatus.NOT_ACCEPTABLE }))
  id: number,
) {
  return this.catsService.findOne(id);
}
```

其他的 `Parse*` 管道用法类似，可以用在 `@Param()`, `@Query()`, `@Body()` 等地方。比如验证 UUID：

```typescript
import { ParseUUIDPipe } from '@nestjs/common';

@Get(':uuid')
async findOneByUUID(@Param('uuid', new ParseUUIDPipe()) uuid: string) {
  // 如果传入的 uuid 格式不对，这里也会自动报错
  return this.catsService.findOne(uuid);
}

// 你还可以指定 UUID 版本，比如只接受 v4: new ParseUUIDPipe({ version: '4' })
@Get(':uuid')
async findOneByUUID(@Param('uuid', new ParseUUIDPipe({ version: '4' })) uuid: string) {
  return this.catsService.findOne(uuid);
}
```



## 创建自定义管道
虽然内置管道很强大，但有时我们需要根据具体业务需求来定制管道。创建自定义管道很简单，只需要实现 `PipeTransform` 接口：

```typescript
// validation.pipe.ts
import { PipeTransform, Injectable, ArgumentMetadata, BadRequestException } from '@nestjs/common';

@Injectable()
export class SimpleValidationPipe implements PipeTransform {
  transform(value: any, metadata: ArgumentMetadata) {
    console.log('管道接收到的原始值:', value);

    // 这里可以添加你的转换或验证逻辑

    // 如果验证失败，可以抛出异常，比如：
    // if (typeof value !== 'string') {
    //   throw new BadRequestException('Value must be a string.');
    // }

    // 如果一切正常，返回处理后的值（或者原始值）
    return value;
  }
}
```

`transform(value, metadata)` 方法有两个参数：

+ `value`: 当前正在处理的参数值
+ `metadata`: 参数的元数据，包含：
    - `type`: 参数来源（'body', 'query', 'param', 'custom'）
    - `metatype`: 参数类型（String, Number 或自定义 DTO 类）
    - `data`: 传给装饰器的字符串，如 `@Body('user')` 中的 `'user'`



## 用管道做复杂验证
假设我们要创建一个 Cat，需要验证包含 name、age、breed 的对象。有两种主流的验证方案：

```typescript
// create-cat.dto.ts
export class CreateCatDto {
  name: string;
  age: number;
  breed: string;
}

// cats.controller.ts
import { Body, Post, UsePipes } from '@nestjs/common';
import { CreateCatDto } from './dto/create-cat.dto';

@Post()
async create(@Body() createCatDto: CreateCatDto) {
  // 我们希望在这里之前，createCatDto 就已经被验证过了
  this.catsService.create(createCatDto);
}
```

怎么做验证呢？

### 使用 Zod (基于 Schema)
Zod 是一个流行的 Schema 验证库。

1. 安装：`npm install --save zod`
2. 创建 Zod Schema：

```typescript
// shared/schemas/cat.schema.ts
import { z } from 'zod';

export const createCatSchema = z
  .object({
    name: z.string().min(1), // 名字不能为空
    age: z.number().int().positive(), // 年龄必须是正整数
    breed: z.string().min(1), // 品种不能为空
  })
  .required(); // 整个对象是必需的

// 从 Schema 推断出 TypeScript 类型
export type CreateCatDto = z.infer<typeof createCatSchema>;
```

3. 创建 Zod 验证管道：

```typescript
// common/pipes/zod-validation.pipe.ts
import { PipeTransform, ArgumentMetadata, BadRequestException } from '@nestjs/common';
import { ZodSchema } from 'zod';

export class ZodValidationPipe implements PipeTransform {
  constructor(private schema: ZodSchema) {}

  transform(value: unknown, metadata: ArgumentMetadata) {
    try {
      const parsedValue = this.schema.parse(value);
      return parsedValue; // 验证通过，返回解析后的值
    } catch (error) {
      // Zod 抛出的错误信息比较详细，可以考虑直接抛出或格式化
      throw new BadRequestException('Validation failed: ' + error.errors.map(e => `${e.path.join('.')} ${e.message}`).join(', '));
    }
  }
}
```

4. 在控制器方法上使用 `@UsePipes` 绑定管道：

```typescript
// cats.controller.ts
import { ZodValidationPipe } from '../common/pipes/zod-validation.pipe';
import { createCatSchema, CreateCatDto } from '../shared/schemas/cat.schema'; // 注意这里 DTO 类型也从 schema 导入

@Post()
@UsePipes(new ZodValidationPipe(createCatSchema)) // 在方法级别应用管道
async create(@Body() createCatDto: CreateCatDto) { // 参数类型使用 Zod 推断出的类型
  this.catsService.create(createCatDto);
}
```

使用 Zod 可能需要在 `tsconfig.json` 中启用 `strictNullChecks`。

启用 `strictNullChecks` 能让 Zod 推断的 TypeScript 类型更精确，并在编译时就帮你发现潜在的 `null` 或 `undefined` 问题，从而减少运行时错误。

### 使用 class-validator (基于装饰器)
这是 Nest 官方文档推荐的方式，与 TypeScript 结合得很好：

1. 安装：`npm i --save class-validator class-transformer`
2. 给 DTO 类加上装饰器：

```typescript
// create-cat.dto.ts
import { IsString, IsInt, Min, MinLength } from 'class-validator';

export class CreateCatDto {
  @IsString()
  @MinLength(1) // 名字不能为空
  name: string;

  @IsInt()
  @Min(1) // 年龄最小为 1
  age: number;

  @IsString()
  @MinLength(1) // 品种不能为空
  breed: string;
}
```

3. 使用内置的 ValidationPipe：

```typescript
// cats.controller.ts
import { ValidationPipe } from '@nestjs/common'; // 导入内置的 ValidationPipe

@Post()
async create(
  @Body(new ValidationPipe()) createCatDto: CreateCatDto, // 在 @Body 装饰器里应用管道
) {
  this.catsService.create(createCatDto);
}
```

或者，更常见的做法是，把它设置成**全局管道**。



## 全局管道
如果验证逻辑在整个应用中都适用，可以设置全局管道，避免重复配置：

```typescript
// main.ts
import { NestFactory } from '@nestjs/core';
import { AppModule } from './app.module';
import { ValidationPipe } from '@nestjs/common';

async function bootstrap() {
  const app = await NestFactory.create(AppModule);

  // 设置全局管道
  app.useGlobalPipes(new ValidationPipe({
    whitelist: true, // 自动移除 DTO 中没有定义的属性
    forbidNonWhitelisted: true, // 如果有多余属性，直接报错
    transform: true, // 自动将传入的数据转换为 DTO 实例 (比如把查询参数的字符串转成数字)
    transformOptions: {
      enableImplicitConversion: true, // 允许基于 TS 类型的隐式转换
    },
  }));

  await app.listen(process.env.PORT ?? 3000);
}
bootstrap();
```

如果管道需要依赖注入，可以通过模块 Provider 方式注册：

```typescript
// app.module.ts
import { Module } from '@nestjs/common';
import { APP_PIPE } from '@nestjs/core';
import { ValidationPipe } from '@nestjs/common'; // 假设 ValidationPipe 在这里或者被导入

@Module({
  // ... 其他配置
  providers: [
    {
      provide: APP_PIPE, // 固定 Token
      useClass: ValidationPipe, // 使用哪个管道类
    },
  ],
})
export class AppModule {}
```

****

## 处理可选参数：DefaultValuePipe
查询参数经常是可选的。比如 `/cats?activeOnly=true&page=1`，如果用户不传 `page`，我们希望默认为 0。但直接用 `ParseIntPipe` 处理 `undefined` 值会报错。

这时 `DefaultValuePipe` 就很有用了，它要放在其他管道之前：

```typescript
import { DefaultValuePipe, ParseBoolPipe, ParseIntPipe } from '@nestjs/common';

@Get()
async findAll(
  @Query('activeOnly', new DefaultValuePipe(false), ParseBoolPipe) activeOnly: boolean,
  @Query('page', new DefaultValuePipe(0), ParseIntPipe) page: number,
) {
  // 如果请求是 /cats，那么 activeOnly 会是 false，page 会是 0
  // 如果请求是 /cats?activeOnly=true，那么 activeOnly 是 true，page 是 0
  // 如果请求是 /cats?page=2，那么 activeOnly 是 false，page 是 2
  return this.catsService.findAll({ activeOnly, page });
}
```



## 内置管道示例
Nest 内置的 Pipe：

+ ValidationPipe
+ ParseIntPipe
+ ParseBoolPipe
+ ParseArrayPipe
+ ParseUUIDPipe
+ DefaultValuePipe
+ ParseEnumPipe
+ ParseFloatPipe
+ ParseFilePipe

创建项目：

```bash
nest new pipe-test -p npm
```

### ParseIntPipe
`ParseIntPipe` 转换参数为整数，无法转换或者转换后不是整数则抛出异常：

![](https://cdn.nlark.com/yuque/0/2024/png/21596389/1708918504892-c5cb3b85-ea05-448e-909d-b4d995b2d3f7.png)

访问页面可以看出 age 是字符串，因为字符串 + 数字才会拼接：

![](https://cdn.nlark.com/yuque/0/2024/png/21596389/1708918554850-37a0d8f6-bb44-4491-b5fe-ba7fcfb2f09a.png)

我们使用 `ParseIntPipe` ：

![](https://cdn.nlark.com/yuque/0/2024/png/21596389/1708918629100-73c4b44f-5bf7-4b98-ad59-07f0d1236c31.png)

正确相加：

![](https://cdn.nlark.com/yuque/0/2024/png/21596389/1708918647386-e2d1870d-e7be-4681-8692-8fc7b5597608.png)

传递一个无法转换整数的会报错：

![](https://cdn.nlark.com/yuque/0/2024/png/21596389/1708918705736-c57ad3ea-101c-4cf4-82db-27f44eb707fe.png)

我们还可以自定义错误：

![](https://cdn.nlark.com/yuque/0/2024/png/21596389/1708918848499-b8a0a659-60d7-45d3-aca6-e04c0e681091.png)

重新访问：

![](https://cdn.nlark.com/yuque/0/2024/png/21596389/1708918875294-192162c6-fa97-4a3f-b5cf-9f4712317a2c.png)

这里也可以通过 exceptionFactory 选项自定义异常工厂函数，抛出异常，让异常过滤器自己处理。

### ParseFloatPipe
`ParseFloatPipe` 是将参数转换为浮点数：

![](https://cdn.nlark.com/yuque/0/2024/png/21596389/1708919184905-ab6878a2-09d3-46c5-a605-a88455700858.png)

访问下：

![](https://cdn.nlark.com/yuque/0/2024/png/21596389/1708919240322-45205f22-de9c-4605-a126-dcd6f553f25a.png)

同样支持  new ParseFloatPipe 的形式，可传入 errorHttpStatusCode 和 exceptionFactory 选项自定义错误。

### ParseBoolPipe
`ParseBoolPipe` 用于将输入的字符串转换为布尔值。如果输入的值无法转换为布尔值，则会抛出异常。

![](https://cdn.nlark.com/yuque/0/2024/png/21596389/1708932264846-b7476564-2b10-4953-b4b4-e155cd280de6.png)

### ParseEnumPipe
`ParseEnumPipe` 用于确保输入的值是特定枚举中的一个有效值。如果输入的值不在枚举中，则会抛出异常。

![](https://cdn.nlark.com/yuque/0/2024/png/21596389/1708932976521-09ff7aee-d859-4fe5-8e96-3e611c90d3be.png)

![](https://cdn.nlark.com/yuque/0/2024/png/21596389/1708933000007-bc378202-e39c-40af-825b-9dca745ba262.png)

如果 role 的值不是 admin 或 user，则会抛出 BadRequestException。

也同样可以通过 errorHttpStatusCode 和 exceptionFactory 来自定义错误。

### ParseUUIDPipe
ParseUUIDPipe 用于验证输入的字符串是否是有效的 UUID（通用唯一标识符）。如果不是有效的 UUID，则会抛出异常。

我们用 uuid 包可以生成 uuid：

![](https://cdn.nlark.com/yuque/0/2023/png/21596389/1687082116898-e8838786-5a3c-4aaf-a66f-806db4ec5a98.png)

![](https://cdn.nlark.com/yuque/0/2024/png/21596389/1708933172594-7eb03c63-78fc-40ed-a39d-1eac2d24710c.png)

![](https://cdn.nlark.com/yuque/0/2024/png/21596389/1708933876802-35492d6b-28ca-4049-8f33-d13c5a09bb90.png)

### DefaultValuePipe
DefaultValuePipe 用于为参数提供一个默认值：

![](https://cdn.nlark.com/yuque/0/2024/png/21596389/1708934065988-a2ee5b50-5327-45cb-846b-dfcc4c087ff9.png)

![](https://cdn.nlark.com/yuque/0/2024/png/21596389/1708934099936-90b6fd8b-8b8d-404b-8c29-b178aef72ca8.png)

### ParseArrayPipe
用于将客户端发送的字符串数组解析为实际的数组类型：

先安装 class-validator 包，这是可以用装饰器和非装饰器两种方式对 class 属性做验证的库。

再安装 class-transformer 包，它是把普通对象转换为对应的 class 实例的包。

```bash
npm install class-validator class-transformer
```

![](https://cdn.nlark.com/yuque/0/2024/png/21596389/1708934607458-14e8bff0-9616-4ce7-8b4e-57cdd30a17d6.png)

![](https://cdn.nlark.com/yuque/0/2024/png/21596389/1708934568422-93cf58c2-e5d1-40c1-ad7b-5da59d103191.png)

还剩下 ValidationPipe 和 ParseFilePipe，之后再讲。
